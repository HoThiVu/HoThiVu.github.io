        ...
        storage.forEach((element) => {
            if (select == element.category) {
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" onclick="deleteProduct(PRODUCT_TABLE,${element.id})" class="btn btn-danger"><i class="bi bi-trash"></i></button></td>
                </tr>
                `;
            }
        });
...
function editsp(id) {
    ...
    callAPI(`films/${id}`, "GET", null).then((res) => {
        ...
        document.getElementById("edit").innerHTML = `<button type="button" onclick="editok(${menu.id})" class="btn btn-success data-toggle="modal" data-target="#exampleModal"">save</button>`;
    });
}
...
// --------------------------------------------------delete-HUY--------
const deleteProduct = (tableName, id) => {
    axios
        .delete(`${URL_API}/${tableName}/${id}`)
